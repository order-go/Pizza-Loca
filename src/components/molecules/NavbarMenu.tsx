    ...
    return (
        <div ref={menuRef} className="z-50">
            <button className="w-auto h-auto mb-2" onClick={handleNavbarMenuClick}>
                {isMenuOpen? (
                    <IoMdClose className="w-8 h-8 text-fourth transition-all duration-1000 transform rotate-190" />
                ):(
                    <IoMdMenu className="w-8 h-8 text-fourth transition-all duration-1000 transform rotate-0" />
                )}
            </button>
            {isMenuOpen&&(
                <ItemsMenu />
